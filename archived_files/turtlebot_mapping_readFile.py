...
def data_loading(cfg, rays_d, step):
    rgb_path = 'data/turtlebot/rgb_1736238323.jpg'
    depth_path = 'data/turtlebot/depth_1736238323.png'
    #pose_path = 'place_holder'

    # Load depth image. following dataset.py 
    depth_data = cv2.imread(depth_path, cv2.IMREAD_UNCHANGED) # cv2.IMREAD_UNCHANGED raw pixel value without conversion
    #depth_data = depth_data.astype(np.float32) / cfg['cam']['png_depth_scale'] * cfg['data']['sc_factor']
    depth_data = depth_data.astype(np.float32) / 1000.0 # mm to m
    depth_data = torch.from_numpy(depth_data.astype(np.float32))

    # Load RGB image using OpenCV. following dataset.py
    color_data = cv2.imread(rgb_path)
    color_data = cv2.cvtColor(color_data, cv2.COLOR_BGR2RGB)  # Convert from BGR to RGB
    color_data = color_data / 255. # Normalize to [0, 1]
    color_data = torch.from_numpy(color_data.astype(np.float32))

    # Load pose data (assuming it's a NumPy array saved as .npy)
    pose = torch.tensor([   [ 1.8809e-01,  4.8544e-01, -8.5380e-01,  7.8488e-01],
                            [-9.8215e-01,  9.2967e-02, -1.6351e-01,  5.4731e-01],
                            [ 1.0646e-16,  8.6931e-01,  4.9427e-01,  1.0542e+00],
    ...
    batch = {
        ...
        'rgb': color_data.unsqueeze(0), 
        'depth': depth_data.unsqueeze(0),  
...
def turtlebot_mapping(cfg, i):
    ...
    dataset_info = {'num_frames':1000, 'num_rays_to_save':num_rays_to_save, 'H':H, 'W':W }
    ...
    for step in trange(500):
        batch = data_loading(cfg, rays_d, step)
