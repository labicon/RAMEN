...
class Mapping():
    ...
    def select_samples(self, samples, valid_indices):
        '''
        randomly select samples from the image, excluding pixels with depth 0
        '''
        indice = random.sample(valid_indices, int(samples))
    ...
    def first_frame_mapping(self, batch, n_iters=100):
        ...
        for i in range(n_iters):
            ...
            #TODO
            indice = self.select_samples(self.config['mapping']['sample'], batch['valid_indx'])
        ...
        with torch.no_grad():
            ...
            for model_j, j in self.neighbors:
                for param_i, param_j in zip(self.model.parameters(), model_j):
                    #param_i.add_(self.ds_mat[rid, j])
    ...
    def global_BA(self, batch, cur_frame_id, dist_algorithm):
        ...
        for i in range(self.config['mapping']['iters']):
            ...
            #TODO: Checkpoint...
            idx_cur = random.sample(batch['valid_indx'], max(self.config['mapping']['sample'] // len(self.keyframeDatabase.frame_ids), self.config['mapping']['min_pixels_cur']))
        ...
        self.lag_loss.append(mean_aug_loss)
...
def data_loading(redis_client, rays_d, step, com_every, agent_i, max_depth):
    # send for consensus 
    if step % com_every == 0:
    ...
    # get left and reight images
    limg_data = redis_client.get('l_img')
    l_img = pickle.loads(limg_data)
    rimg_data = redis_client.get('r_img')
    r_img = pickle.loads(rimg_data)

    # Get depth image from Redis
    depth_data = redis_client.get('depth_image')
    depth_image = pickle.loads(depth_data)
    row_idxs, col_idxs = np.nonzero( (depth_image>0) & (depth_image<max_depth) )
    valid_indx = [ row_idx*depth_image.shape[1] + col_idx  for row_idx, col_idx in zip(row_idxs, col_idxs)]
...
def turtlebot_mapping(cfg, i):
    ...
    cfg['mesh']['vis'] = cfg['mesh']['vis']
    cfg['mesh']['voxel_eval'] = cfg['mesh']['voxel']
    cfg['cam']['fx'] = cfg['cam']['fx'] 
    cfg['cam']['fy'] = cfg['cam']['fy'] 
    cfg['cam']['cx'] = cfg['cam']['cx'] 
    cfg['cam']['cy'] = cfg['cam']['cy'] 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dataset', 
                        action='store_true', 
                        help='store batches as replicat scene')
